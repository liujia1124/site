...
class PortalPostModel extends Model
{
    ...
    public function getMaps($arr){
        $rs = $this->field('id,create_time,post_title,more')
            ->where('id','in',$arr)
            ->select();
    ...
    public function getUserArticle($userId, $pageNum, $pagesize=10){
        ...
        $rs = $this->where($where)->limit($start, $pagesize)->select()->toArray();
        return $rs;
    }

    public function addArticle($data){
        return $this->getQuery()->insert($data, false, true);
    }

    public function getArticleId($spiderId, $textId){
        $id = $this->where([ 'spider_id'=>$spiderId, 'spider_text_id'=>$textId])->value('id');
