...
use app\portal\model\PortalAuthModel;
use Qiniu\Config;
...
class PostService
{
    ...
    public function adminPostList($filter, $isPage = false)
    {
        ...
        $auth    = new PortalAuthModel();
        $readAll = $auth->checkAuth($user_id ,'portal/adminarticle/all');
        if(!$readAll&&$user_id !==1){
            //没有权限只能看自己的，不过有一个用户是神
            $where = [
                'a.create_time' => ['>=', 0],
                'a.delete_time' => 0,
                'a.user_id'     => $user_id
        ...
        if (!empty($keyword)) {
            $where['a.post_keywords'] = ['like', "%$keyword%"];
        }
